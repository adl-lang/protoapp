...
export function makeApiRequests(
  ...
  return {
    ...
    refreshTokenApi: input.refreshTokenApi === undefined ? {token : texprRefreshToken(), cap : common_http.texprUnit(), service_prefix : "", service : {refresh : {path : "/refresh", rateLimit : null, reqType : texprRefreshReq(), respType : texprRefreshResp()}}} : input.refreshTokenApi,
    userApi: input.userApi === undefined ? {token : texprAdminAccessToken(), cap : texprCapability(), service_prefix : "", service : {create_user : {path : "/users/create", rateLimit : null, reqType : texprUserDetails(), respType : protoapp_db.texprAppUserId()}, update_user : {path : "/users/update", rateLimit : null, reqType : texprWithId(protoapp_db.texprAppUserId(), texprUserDetails()), respType : common_http.texprUnit()}, query_users : {path : "/users/query", rateLimit : null, reqType : texprQueryUsersReq(), respType : texprPaginated(texprUserWithId())}}} : input.userApi,
  };
}
