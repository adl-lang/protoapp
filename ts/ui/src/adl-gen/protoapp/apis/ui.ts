...
export function makeApiRequests(
  ...
  return {
    ...
    messages: input.messages === undefined ? {new_message : {path : "/messages/new", security : {kind : "token"}, rateLimit : null, reqType : texprNewMessageReq(), respType : protoapp_db.texprMessageId()}, recent_messages : {path : "/messages/recent", security : {kind : "token"}, rateLimit : null, reqType : texprRecentMessagesReq(), respType : texprPaginated(texprMessage())}} : input.messages,
